...
- Season/Cycle change -> re-read the table rows. This fires for EVERY account:
  the selectors, the table and this callback's Output are all in a player's
  render too, so a player browses seasons/cycles exactly like a coach. (Dash
  will not fire a callback whose Inputs/Outputs are missing from the current
  render, which is why the filters must live outside the coach-only section.)
  Cycle (Fall/Spring, `velo_board.VELO_CYCLES`) is independent of Season --
  always one of two fixed values regardless of which season is picked -- so
  unlike the old Week date-picker, there's no "snap it back inside the
  season's bounds" case to handle here.
...
- Save click -> persist (Velo Goal + changed Assessment/velo overrides), then
...
def register_callbacks(dash_app) -> None:
    ...
    @dash_app.callback(
        ...
        Input("velo-cycle", "value"),
        prevent_initial_call=True,
    )
    def _on_season_or_cycle(season, cycle):
        return visual.board_records(velo_board.board_rows(season, cycle))
    ...
    @dash_app.callback(
        ...
        State("velo-cycle", "value"),
        prevent_initial_call=True,
    )
    def _on_save(n_clicks, grid_data, season, cycle):
        if not n_clicks or not _is_coach():
            return no_update, no_update, no_update
        grid.save_board(grid_data, season, cycle, updated_by=getattr(current_user, "id", None))
        rows = visual.board_records(velo_board.board_rows(season, cycle))
