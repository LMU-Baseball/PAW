...
def test_catching_read_matches_compute(rebuilt_catching):
    ...
    comp = catching_caps._compute_slaa_season_rollup(cid)
    for k in ("slaa", "sl_plus", "taken"):
        assert row[k] == comp[k], k


def test_catching_read_missing_returns_none(rebuilt_catching):
    assert precalc.read_catching_season(-1) is None
